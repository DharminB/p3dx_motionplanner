...
class MotionPlanner(object):
    ...
    def update(self, cmd):
        ...
        if self._n_robots == 1:
            self._result = [P3DXTrajectory(x_traj=trajectories['input'][0, :], w_traj=trajectories['input'][1, :])]
        else:
            self._result = [P3DXTrajectory(x_traj=trajectories['vehicle'+str(k)]['input'][0, :], w_traj=trajectories['vehicle'+str(k)]['input'][1, :]) for k in range(self._n_robots)]
